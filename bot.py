    ...
    async def setup_hook(self) -> None:
        ...
        for ext in EXTENSIONS:
            try:
                ...
                log.error("Missing file for %s", ext)
            except Exception:
        ...
        async def on_app_error(interaction: discord.Interaction, error: app_commands.AppCommandError) -> None:
            ...
            if isinstance(error, app_commands.CheckFailure):
                await ack(interaction, str(error) or "You cannot use that command.")
                return
    ...
    async def _sync_commands(self) -> None:
        for guild in list(self.guilds):
            try:
                ...
                log.warning("Cannot clear guild slash commands in %s", guild.id)
            except discord.HTTPException:
                log.exception("Failed clearing guild commands in %s", guild.id)
        try:
